...
export default function UserPanel() {
  ...
  // --- BÚSQUEDA Y FILTRADO DE CLUBES (RR-010) ---
  ...
  // Lista de géneros disponibles, derivada dinámicamente de los clubes cargados
  const generosDisponibles = [
    "Todos",
    ...Array.from(new Set(clubes.map(c => c.generoLiterario).filter(Boolean)))
  ]

  // Clubes filtrados por texto de búsqueda (nombre/descripción) y por género
  const clubesFiltrados = clubes.filter(club => {
    ...
    const coincideTexto =
      ...
      club.descripcion?.toLowerCase().includes(texto)

    const coincideGenero =
      generoSeleccionado === "Todos" || club.generoLiterario === generoSeleccionado
  ...
  return (
    <section className="min-h-screen bg-[#f8f9fa] py-8 px-4 font-sans sm:px-6">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {/* --- BÚSQUEDA Y FILTRADO DE CLUBES (RR-010) --- */}
        <div className="bg-white rounded-3xl border border-gray-100 p-4 md:p-5 shadow-2xs flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            <MdSearch size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-300" />
            <input
              type="text"
              value={busqueda}
              onChange={(e) => setBusqueda(e.target.value)}
              placeholder="Buscar club por nombre o descripción..."
              className="w-full pl-10 pr-9 py-2.5 rounded-xl border border-gray-200 bg-gray-50/50 text-xs font-semibold text-[#2c3e50] focus:outline-none focus:ring-2 focus:ring-amber-200 focus:border-amber-300 transition"
            />
            {busqueda && (
              <button
                type="button"
          ...
          <select
            ...
            className="rounded-xl border border-gray-200 bg-gray-50/50 text-xs font-black uppercase tracking-wider text-[#2c3e50] px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-amber-200 focus:border-amber-300 transition cursor-pointer"
          >
        ...
        {clubesFiltrados.length === 0 && (
          <div className="bg-white rounded-3xl border border-dashed border-gray-200 p-10 text-center">
            <MdSearch size={32} className="mx-auto text-gray-200 mb-3" />
