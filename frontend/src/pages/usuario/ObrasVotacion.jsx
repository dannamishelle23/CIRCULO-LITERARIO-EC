...
export default function ObrasVotacion() {
  ...
  // Filtrado por título o autor (RR-010), aplicado sobre la pestaña activa
  ...
  const obrasVotacionFiltradas = filtrarPorBusqueda(obrasVotacion)
  const obrasPublicadasFiltradas = filtrarPorBusqueda(obrasPublicadas)
  ...
  return (
    <section className="min-h-screen bg-[#f8f9fa] py-8 px-4 font-sans sm:px-6">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* --- BÚSQUEDA DE OBRAS POR TÍTULO O AUTOR (RR-010) --- */}
        <div className="relative">
          <MdSearch size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300" />
          <input
            type="text"
            value={busquedaObra}
            onChange={(e) => setBusquedaObra(e.target.value)}
            placeholder="Buscar obra por título o autor..."
            className="w-full pl-11 pr-9 py-3 rounded-2xl border border-gray-100 bg-white shadow-2xs text-xs font-semibold text-[#2c3e50] focus:outline-none focus:ring-2 focus:ring-amber-200 focus:border-amber-300 transition"
          />
          {busquedaObra && (
            <button
              type="button"
        ...
        <div className="bg-white rounded-3xl border border-gray-100 p-6 md:p-8 shadow-2xs min-h-[400px]">
          ...
          {activeTab === "votacion" && (
            <div className="space-y-6">
              ...
              {obrasVotacionFiltradas.length === 0 ? (
                <div className="text-center py-20 bg-gray-50/50 rounded-3xl border-2 border-dashed border-gray-100">
                  ...
                  <p className="text-xs font-black text-gray-400 uppercase tracking-widest">
                    {busquedaObra ? "Ninguna obra coincide con tu búsqueda" : "No hay manuscritos en votación"}
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {obrasVotacionFiltradas.map((obra) => {
              ...
              {obrasPublicadasFiltradas.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-xs font-black text-gray-400 uppercase">
                    {busquedaObra ? "Ninguna obra coincide con tu búsqueda" : "Aún no hay obras publicadas"}
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
