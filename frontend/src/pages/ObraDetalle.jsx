...
import { obtenerObra, votarObra, likeObra, calificarObra } from "../services/obraService";
...
export default function ObraDetalle() {
  ...
  // --- CALIFICACIÓN POR ESTRELLAS (RR-009) ---
  ...
  const cargarObra = async () => {
    try {
      ...
      // El backend ya calcula estos valores en el servidor (GET /obras/:id),
      // por lo que aquí solo los reflejamos tal cual llegan.
      setVotos(obraData.votos || 0);
      setYaVoto(Boolean(obraData.yaVoto));

      setLikes(obraData.likes || 0);
      setYaDioLike(Boolean(obraData.yaDioLike));

      setPromedioCalificacion(obraData.calificacionPromedio || 0);
      setTotalCalificaciones(obraData.totalCalificaciones || 0);
      setMiCalificacion(obraData.miCalificacion || 0);
  ...
  // --- LIKE (RR-009): botón funcional, ya no está fijo en 0 ---
  const handleLike = async () => {
    ...
    try {
      ...
      const nuevoEstado = !yaDioLike;
      setYaDioLike(nuevoEstado);
      setLikes((prev) => (nuevoEstado ? prev + 1 : Math.max(0, prev - 1)));

      const res = await likeObra(id);

      // Si el backend devuelve el conteo real, lo usamos como fuente de verdad
      if (typeof res.likes === "number") {
        setLikes(res.likes);
      }
      if (typeof res.yaDioLike === "boolean") {
        setYaDioLike(res.yaDioLike);
      }

    } catch (error) {
      // Revertimos el optimismo si falló la petición
      setYaDioLike((prev) => !prev);
      setLikes((prev) => (yaDioLike ? prev + 1 : Math.max(0, prev - 1)));

      const msg = error.response?.data?.msg || "No se pudo registrar tu like en este momento";
      setTipoMensaje("error");
      setMensaje(msg);
      setTimeout(() => setMensaje(""), 4000);
    } finally {
  ...
  const handleCalificar = async (estrellas) => {
    if (cargandoCalificacion) return;

    try {
      setCargandoCalificacion(true);

      const res = await calificarObra(id, estrellas);

      setMiCalificacion(estrellas);
      setPromedioCalificacion(res.promedio ?? promedioCalificacion);
      setTotalCalificaciones(res.totalCalificaciones ?? totalCalificaciones);

      setTipoMensaje("success");
      setMensaje(res.msg || "¡Gracias por calificar esta obra!");
      setTimeout(() => setMensaje(""), 4000);

    } catch (error) {
      const msg = error.response?.data?.msg || "No se pudo registrar tu calificación";
      setTipoMensaje("error");
      setMensaje(msg);
      setTimeout(() => setMensaje(""), 4000);
    } finally {
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-2xs text-center space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  className={`rounded-xl p-3 text-left transition cursor-pointer active:scale-95 border ${
                    yaDioLike
                      ? "bg-rose-50 border-rose-100"
                      : "bg-gray-50 border-gray-100 hover:bg-rose-50/50"
                  }`}
                >
                  <p className={`text-[10px] font-black uppercase tracking-wider ${yaDioLike ? "text-rose-500" : "text-gray-400"}`}>
                    Likes
                  </p>
                  <p className={`text-xl font-black mt-1 ${yaDioLike ? "text-rose-600" : "text-gray-700"}`}>
                    {likes}
                  </p>
                </button>
              </div>
            ...
            {/* PANEL DE CALIFICACIÓN POR ESTRELLAS (RR-009) */}
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-2xs text-center space-y-4">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400 pb-2 border-b border-gray-50 text-left">
              ...
              <div className="flex items-center justify-center gap-3">
                <StarRating value={promedioCalificacion} readOnly size={20} />
              ...
              {user ? (
                <div className="pt-2 border-t border-gray-50 space-y-2">
                  <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">
                    {miCalificacion > 0 ? "Tu calificación" : "Califica esta obra"}
                  </p>
                ...
                <p className="text-[9px] font-bold text-gray-300 uppercase tracking-widest pt-2 border-t border-gray-50">
