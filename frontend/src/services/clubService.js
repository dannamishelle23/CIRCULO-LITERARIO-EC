...
/* BUSCAR / FILTRAR CLUBES (RR-010)
   Intenta usar el endpoint de búsqueda del backend (?nombre=&genero=).
   Si el backend aún no lo implementa (404/501), cae de vuelta a listar
   todos los clubes para que el filtrado se resuelva en el cliente. */
export const buscarClubs = async ({ nombre, genero } = {}) => {
  try {
    const response = await api.get("/clubes/buscar-clubes", {
      params: { nombre, genero },
    });
    return response.data.clubes;
  } catch (error) {
